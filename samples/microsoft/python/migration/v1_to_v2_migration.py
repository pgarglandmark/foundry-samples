...
try:
    ...
    print(
        "⚠️  Warning: azure-ai-projects package not available. Project endpoint functionality disabled."
    )
...
SUBSCRIPTION_ID = (
    os.getenv("AGENTS_SUBSCRIPTION") or "921496dc-987f-410f-bd57-426eb2611356"
)
...
WORKSPACE_V2 = (
    os.getenv("AGENTS_WORKSPACE_V2")
    or "e2e-tests-westus2-account@e2e-tests-westus2@AML"
)
...
SOURCE_TENANT = (
    os.getenv("SOURCE_TENANT")
    or os.getenv("AGENTS_TENANT")
    or "72f988bf-86f1-41af-91ab-2d7cd011db47"
)  # Microsoft tenant

# Production Resource Configuration
PRODUCTION_RESOURCE = os.getenv("PRODUCTION_RESOURCE")  # e.g., "nextgen-eastus"
PRODUCTION_SUBSCRIPTION = os.getenv(
    "PRODUCTION_SUBSCRIPTION"
)  # e.g., "b1615458-c1ea-49bc-8526-cafc948d3c25"
PRODUCTION_TENANT = os.getenv(
    "PRODUCTION_TENANT"
)  # e.g., "33e577a9-b1b8-4126-87c0-673f197bf624"
PRODUCTION_TOKEN = os.getenv(
    "PRODUCTION_TOKEN"
)  # Production token from PowerShell script
...
def get_production_v2_base_url(
    resource_name: str, subscription_id: str, project_name: str
) -> str:
    """
...
def get_token_from_az(tenant_id: Optional[str] = None) -> Optional[str]:
    ...
    try:
        ...
        if proc.returncode != 0:
            print(
                "az CLI returned non-zero exit code when fetching token:",
                proc.stderr.strip(),
            )
            return None
        ...
        print(
            "az CLI not found on PATH. Please install Azure CLI or set AZ_TOKEN env var."
        )
        return None
...
class ManualAzureCliCredential:
    ...
    def get_token(self, *scopes, **kwargs):
        ...
        try:
            ...
            for line in reversed(lines):
                if (
                    line.startswith("ey") or len(line) > 50
                ):  # JWT tokens start with 'ey' or are long strings
                    token = line
                    break
...
def get_azure_credential():
    ...
    if not PROJECT_CLIENT_AVAILABLE:
        raise ImportError(
            "azure-identity package is required for credential functionality"
        )
    ...
    if is_container:
        ...
        try:
            print(
                "🐳 Container environment detected, using DefaultAzureCredential for better compatibility"
            )
            ...
            try:
                ...
                print(
                    "💡 This might be due to Azure CLI version mismatch between host and container"
                )
                raise Exception(
                    "All credential methods failed. Host CLI: 2.77.0, Container CLI: 2.78.0. Try: az upgrade"
                )
    else:
...
def do_api_request_with_token(
    ...
    if "localhost" in url or "host.docker.internal" in url:
        ...
        host_type = (
            "localhost" if "localhost" in url else "host.docker.internal (Docker)"
        )
        print(
            f"🏠 Making request to {host_type} with extended timeout and no SSL verification: {url}"
        )
...
def ensure_project_connection_package():
    ...
    try:
        ...
        if hasattr(AIProjectClient, "from_connection_string"):
            ...
            print(
                "⚠️  Current azure-ai-projects version doesn't support from_connection_string"
            )
...
def get_assistant_from_project_connection(
    project_connection_string: str, assistant_id: str
) -> Dict[str, Any]:
    ...
    if not PROJECT_CLIENT_AVAILABLE:
        print(
            "❌ azure-ai-projects package is required for project connection string functionality"
        )
        ...
        try:
            ...
            raise ImportError(
                "Failed to import AIProjectClient after installation"
            )  # Ensure we have the correct version
    if not ensure_project_connection_package():
        raise ImportError(
            "azure-ai-projects==1.0.0b10 is required for project connection string functionality"
        )

    # Try to use from_connection_string method (available in beta versions)
    try:
        project_client = AIProjectClient.from_connection_string(
            credential=get_azure_credential(), conn_str=project_connection_string
        )
...
def list_assistants_from_project_connection(
...
def get_assistant_from_project(
    ...
    print(
        "   🌐 Using direct API call to project endpoint (bypassing AIProjectClient SDK)"
    )
    ...
    try:
        ...
        if PROJECT_CLIENT_AVAILABLE:
            ...
            if not subscription_id or not resource_group_name or not project_name:
                print(
                    "   🔍 Some project parameters missing, attempting to extract from endpoint or environment..."
                )

                # Use environment variables as fallbacks
                subscription_id = (
                    subscription_id
                    or os.getenv("AGENTS_SUBSCRIPTION")
                    or "921496dc-987f-410f-bd57-426eb2611356"
                )
                resource_group_name = (
                    resource_group_name
                    or os.getenv("AGENTS_RESOURCE_GROUP")
                    or "agents-e2e-tests-eastus"
                )
                ...
                if not project_name:
                    ...
                    if project_match:
                        ...
                        print(
                            f"   📝 Extracted project name from endpoint: {project_name}"
                        )
                    else:
                        project_name = "default-project"
                        print(
                            f"   ⚠️  Could not extract project name from endpoint, using default: {project_name}"
                        )

                print(
                    f"   📋 Using: subscription={subscription_id[:8]}..., resource_group={resource_group_name}, project={project_name}"
                )
            ...
            try:
                ...
                raise RuntimeError(
                    f"Both direct API call and AIProjectClient failed. Direct API error: {e}, AIProjectClient error: {client_error}"
                )
        else:
            raise


def list_assistants_from_project(
    project_endpoint: str,
    subscription_id: Optional[str] = None,
    resource_group_name: Optional[str] = None,
    project_name: Optional[str] = None,
    ...
    try:
        ...
        print(
            f"   ✅ Successfully retrieved {len(agent_list)} assistants via direct API call"
        )
        ...
        if PROJECT_CLIENT_AVAILABLE:
            ...
            try:
                ...
                print(
                    f"   ⚠️  Trying alternative AIProjectClient constructor due to: {type_error}"
                )
                try:
                    project_client = AIProjectClient(
                        endpoint=project_endpoint,
                    ...
                    raise RuntimeError(
                        f"Could not initialize AIProjectClient with any constructor pattern. Original error: {type_error}, Fallback error: {fallback_error}"
                    )
...
def create_agent_version_via_api(
    agent_name: str,
    agent_version_data: Dict[str, Any],
    production_resource: Optional[str] = None,
    production_subscription: Optional[str] = None,
    ...
    if production_resource and production_subscription:
        ...
        base_url = get_production_v2_base_url(
            production_resource, production_subscription, production_resource
        )
    ...
    print(
        f"   Keys: {list(agent_version_data.keys()) if isinstance(agent_version_data, dict) else 'Not a dict'}"
    )
    if isinstance(agent_version_data, dict):
        ...
        print(
            json.dumps(agent_version_data, indent=2, default=str)[:2000] + "..."
            if len(str(agent_version_data)) > 2000
            else json.dumps(agent_version_data, indent=2, default=str)
        )
    ...
    try:
        ...
        if production_resource and PRODUCTION_TOKEN:
            ...
            response = do_api_request_with_token(
                "POST", url, PRODUCTION_TOKEN, params=params, json=agent_version_data
            )
        else:
            print("   🔑 Using standard token for authentication")
            response = do_api_request(
                "POST", url, params=params, json=agent_version_data
            )
        result = response.json()
...
def v1_assistant_to_v2_agent(
    v1_assistant: Dict[str, Any], agent_name: Optional[str] = None, version: str = "1"
) -> Dict[str, Any]:
    """
    ...
    if unsupported_tools:
        print(
            f"   📋 Unsupported tools that will be skipped: {', '.join(unsupported_tools)}"
        )

    # Derive agent name if not provided
    if not agent_name:
        agent_name = (
            v1_assistant.get("name") or f"agent_{v1_assistant.get('id', 'unknown')}"
        )
    ...
    if not isinstance(v1_metadata, dict):
        print(
            f"   ⚠️  Warning: metadata is not a dict (type: {type(v1_metadata)}), using empty dict"
        )
        v1_metadata = {}
    ...
    elif "internal_metadata" in v1_assistant and isinstance(
        v1_assistant["internal_metadata"], dict
    ):
    ...
    if feature_flags and isinstance(feature_flags, dict):
        ...
        print(
            f"   🚩 Preserving {len(feature_flags)} feature flags: {list(feature_flags.keys())}"
        )
    ...
    if isinstance(v1_tool_resources, str):
        try:
            ...
            try:
                v1_tool_resources = (
                    eval(v1_tool_resources)
                    if v1_tool_resources.strip().startswith("{")
                    else {}
                )
            except (SyntaxError, ValueError, NameError):
                print(
                    f"   ⚠️  Could not parse tool_resources string: {v1_tool_resources}"
                )
                v1_tool_resources = {}
        ...
        if isinstance(tool, dict):
            ...
            if tool_type == "file_search" and "file_search" in v1_tool_resources:
                ...
                if "vector_store_ids" in file_search_resources:
                    transformed_tool["vector_store_ids"] = file_search_resources[
                        "vector_store_ids"
                    ]
                    print(
                        f"     Added vector_store_ids: {file_search_resources['vector_store_ids']}"
                    )

            # Handle code_interpreter tool
            elif (
                tool_type == "code_interpreter"
                and "code_interpreter" in v1_tool_resources
            ):
                ...
                if "file_ids" in code_resources:
                    ...
                    print(
                        f"     Added container with auto type and file_ids: {code_resources['file_ids']}"
                    )
                else:
            ...
            # Handle MCP tools
                ...
                print(
                    f"     Added MCP tool properties: {[k for k in tool.keys() if k != 'type' and tool[k] is not None]}"
                )
            ...
            # Handle computer_use_preview tools
                ...
                for key in ["display_width", "display_height", "environment"]:
                    if key in tool:
                        transformed_tool[key] = tool[key]
                print(
                    f"     Added computer use tool properties: {[k for k in tool.keys() if k != 'type']}"
                )
            ...
            # Handle image_generation tools
                ...
                print(
                    f"     Added image generation tool properties: {[k for k in tool.keys() if k != 'type']}"
                )
            ...
            # Handle azure_function tools
                ...
                for key in [
                    ...
                    if key in tool:
                        transformed_tool[key] = tool[key]
                print(
                    f"     Added Azure Function tool properties: {[k for k in tool.keys() if k != 'type']}"
                )
            ...
            # Handle any other tool types by copying all properties except 'type'
                ...
                for key, value in tool.items():
                    if key != "type":
                        transformed_tool[key] = value
                print(
                    f"     Added generic tool properties for {tool_type}: {[k for k in tool.keys() if k != 'type']}"
                )
...
def save_v2_agent_to_cosmos(
    ...
    if not project_id:
        project_id = (
            "e2e-tests-westus2-account@e2e-tests-westus2@AML"  # Default from example
        )
    ...
    object_structure = {
        ...
        "metadata": v2_data.get(
            "metadata", {}
        ),  # Original agent metadata (without v1 ID)
    ...
    if isinstance(agent_version_doc["object"], dict):
        ...
        print(
            f"   ❌ ERROR: 'object' field is not a dict: {agent_version_doc['object']}"
        )
    ...
    try:
        ...
        print(
            f"✅ Successfully saved v2 agent '{v2_agent_data['v2_agent_object']['name']}' to Cosmos DB"
        )
        print(f"   - Agent Version: {agent_version_doc['id']}")
        print(f"   - Migration Metadata: {migration_doc['id']}")

        return {"agent_version": agent_version_result, "migration": migration_result}
    except Exception as e:
...
def process_v1_assistants_to_v2_agents(
    args=None,
    ...
    project_subscription: Optional[str] = None,
    project_resource_group: Optional[str] = None,
    project_name: Optional[str] = None,
    production_resource: Optional[str] = None,
    production_subscription: Optional[str] = None,
    production_tenant: Optional[str] = None,
    ...
    need_beta_version = (
        os.environ.get("NEED_BETA_VERSION") == "true"
        or project_connection_string is not None
    )

    if need_beta_version:
        print(
            "🔧 Project connection string detected - ensuring beta version is installed..."
        )
        if not ensure_project_connection_package():
    ...
    if project_connection_string:
        ...
        if not PROJECT_CLIENT_AVAILABLE:
            print(
                "❌ Error: azure-ai-projects package is required for project connection string functionality"
            )
        ...
        if assistant_id:
            print(
                f"🎯 Fetching specific assistant from project connection: {assistant_id}"
            )
            try:
                assistant_data = get_assistant_from_project_connection(
                    project_connection_string, assistant_id
                )
                v1_assistants = [assistant_data]
            except Exception as e:
                print(
                    f"❌ Failed to fetch assistant {assistant_id} from project connection: {e}"
                )
                return
        else:
            print("📊 Fetching all assistants from project connection")
            try:
                v1_assistants = list_assistants_from_project_connection(
        ...
        print(
            f"📊 Found {len(v1_assistants)} v1 assistant records from project connection"
        )
        ...
        if not PROJECT_CLIENT_AVAILABLE:
            print(
                "❌ Error: azure-ai-projects package is required for project endpoint functionality"
            )
        ...
        if assistant_id:
            ...
            try:
                assistant_data = get_assistant_from_project(
                    project_endpoint,
                    assistant_id,
            ...
            try:
                v1_assistants = list_assistants_from_project(
                    project_endpoint,
                    project_subscription,
                    project_resource_group,
                    project_name,
        ...
        print(
            f"📖 Reading v1 assistants from Cosmos DB: {DATABASE_NAME}/{SOURCE_CONTAINER}"
        )
        ...
        if not connection_string:
            print(
                "Error: COSMOS_CONNECTION_STRING environment variable must be set or provided as parameter"
            )
            print(
                "Set it with: $env:COSMOS_CONNECTION_STRING='AccountEndpoint=...;AccountKey=...'"
            )
            print(
                "Or provide it as command line argument: python v1_to_v2_migration.py <assistant_id> <cosmos_connection_string>"
            )
            sys.exit(1)
    ...
    if not TOKEN and not set_api_token(
        force_refresh=force_refresh, tenant_id=tenant_for_auth
    ):
    ...
    for idx, v1_assistant in enumerate(v1_assistants):
        try:
            ...
            if project_connection_string:
                print(
                    f"   ✅ Processing Project Connection data for assistant: {v1_assistant.get('id', 'unknown')}"
                )
            elif project_endpoint:
                print(
                    f"   ✅ Processing Project Endpoint data for assistant: {v1_assistant.get('id', 'unknown')}"
                )
            elif use_api:
                print(
                    f"   ✅ Processing API data for assistant: {v1_assistant.get('id', 'unknown')}"
                )
            else:
                print(
                    f"   ✅ Processing Cosmos DB data for assistant: {v1_assistant.get('id', 'unknown')}"
                )
            ...
            if args:
                ...
                if hasattr(args, "add_test_function") and args.add_test_function:
                    ...
                    print(
                        f"   ✅ Added test function tool: {test_function_tool['function']['name']}"
                    )
                ...
                if hasattr(args, "add_test_imagegen") and args.add_test_imagegen:
                    ...
                    test_imagegen_tool = {"type": "image_generation"}
                    ensure_tools_array()
                ...
                if hasattr(args, "add_test_computer") and args.add_test_computer:
                    ...
                    print(
                        f"   ✅ Added test computer use tool: {test_computer_tool['environment']} environment"
                    )

                # Add test Azure Function tool
                if (
                    hasattr(args, "add_test_azurefunction")
                    and args.add_test_azurefunction
                ):
                    ...
                    print(
                        f"   ✅ Added test Azure Function tool: {test_azurefunction_tool['name']} (using Azurite at {storage_service_endpoint})"
                    )
            ...
            # project_id matches existing data format

            # Extract feature flags to pass to save function
            v1_metadata = v1_assistant.get("metadata", {})
            if "feature_flags" in v1_metadata:
                # Feature flags will be extracted from metadata
                pass
            elif "internal_metadata" in v1_assistant and isinstance(
                v1_assistant["internal_metadata"], dict
            ):
                # Feature flags will be extracted from internal_metadata
                pass
            ...
            if production_resource and not PRODUCTION_TOKEN:
                print(
                    "❌ Production resource specified but no PRODUCTION_TOKEN environment variable found. Skipping v2 API save."
                )
                print(
                    "💡 Use run-migration-docker-auth.ps1 for automatic dual-token authentication"
                )
                continue

            api_result = create_agent_version_via_api(
                agent_name, api_payload, production_resource, production_subscription
            )
            ...
            print(
                f"   Assistant data keys: {list(v1_assistant.keys()) if v1_assistant else 'N/A'}"
            )
            continue
...
Examples:
  ...
  python v1_to_v2_migration.py --project-endpoint "https://source-project.api.azure.com/api/projects/p-3" --production-resource "nextgen-eastus" --production-subscription "b1615458-c1ea-49bc-8526-cafc948d3c25" --production-tenant "33e577a9-b1b8-4126-87c0-673f197bf624" asst_abc123
    ...
    assistant_id = (
        args.assistant_id if args.assistant_id and args.assistant_id.strip() else None
    )
    cosmos_connection_string = (
        args.cosmos_endpoint
        if args.cosmos_endpoint and args.cosmos_endpoint.strip()
        else None
    )
    ...
    if args.production_resource:
        print(
            f"🏭 Saving agents via PRODUCTION v2 API (resource: {args.production_resource})"
        )
    ...
    process_v1_assistants_to_v2_agents(
        ...
        args.project_connection_string,
        args.project_subscription,
        args.project_resource_group,
        args.project_name,
        args.production_resource,
        args.production_subscription,
        args.production_tenant,
        args.source_tenant,
    )
